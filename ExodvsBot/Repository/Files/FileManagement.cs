...
namespace ExodvsBot.Repository.Files
{
    public class FileManagement
    {
        ...
        public async static Task<OcorrenciaDto?> GetLastLine()
        {
            ...
            if (parts.Length < 5)
            {
                return null; // Retorna nulo se a linha tiver formato incorreto
            }
                ...
                parts[2].Trim().Equals("\"Buy\"", StringComparison.OrdinalIgnoreCase))
            {
                ...
                {
                    // Se houver erro de conversão, retorna nulo
                    return null;
                }
            }

            return null; // Retorna nulo se a primeira linha não for "True" e "Buy"
        }
